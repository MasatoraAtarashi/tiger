...
/**
 * Headless (non-interactive) version of Tiger CLI for testing and scripting
 */
...
if (args.length === 0) {
  console.error('Usage: tiger-cli-headless "your prompt here"');
  process.exit(1);
}

const userInput = args.join(' ');

// TypeScriptのtigerモジュールを実行
const runTigerChat = async (userInput) => {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('npx', ['ts-node', '--transpile-only', '-e', `
      const { tigerChat } = require('${projectRoot}/src/tiger');
